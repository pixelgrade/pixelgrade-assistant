...
class Pixassist_Nova_Menu {
	...
	public function nova_menu_shortcode_html( $atts ) {
		...
		if ( $query->have_posts() ) { ?>
			...
			<p class="jetpack-nova-menu-shortcode no-items"><?php esc_html_e( 'You seem to be short on menu entries. You can start creating them on your dashboard.', '__plugin_txtd' ); ?></p>
			<?php
		}
		$html = ob_get_clean();
	...
	public function the_menu_item_labels( $post_id, $exclude_labels = array() ) {
		...
		$html = '<div class="menu-item-labels"><span class="label-text screen-reader-text">' . esc_html__( 'Labels', '__plugin_txtd' ) . ':</span>';
		$labels = array();
		// Loop through all the labels
		foreach ( $menu_item_labels as $menu_item_label ) {
