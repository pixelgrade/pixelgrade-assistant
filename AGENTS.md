...
- **Docs-panel escalation SlotFill** — scope `pixelgrade-docs`; the editor docs bundle (#46) exposes `window.pixelgradeAdminHub.docs` with `{ EscalationSlot, EscalationFill }` under script handle `pixelgrade-docs`, and Plus may enhance requests through the JS filter `pixelgrade.docs.ticketRequest`.
