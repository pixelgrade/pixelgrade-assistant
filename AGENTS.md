...
- **`pixelgrade.adminHub.stylesSections` (JS filter, `admin/src-modern/hub/tabs/Styles.js`)** — same pattern as `accountPanels`, for the Styles tab: `applyFilters( 'pixelgrade.adminHub.stylesSections', [], stylesData )` (`@wordpress/hooks`) runs after the host-owned destinations. Descriptors: `id` (string, sanitized like account panel ids; first registration of a duplicate wins), `component` (`Function`, receives `{ stylesData, sectionId }`), `order` (`int`, default `10`, ties keep registration order). Renders each in `#pixelgrade-styles-section-<id>` / `data-styles-section="<id>"`, supporting `?tab=styles&section=<id>` deep-link scrolling. Reference consumer: Style Manager's Fonts section for the self-hosted cloud fonts control (`style-manager#183`), a companion outside the Plus/Assistant pair.
