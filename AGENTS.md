...
- **Contributed sections (JS filter pattern, `admin/src-modern/hub/contributedSections.js`)** — same pattern as `accountPanels`, generalized so any hub tab can accept contributed content below its host-owned destinations: `getContributedSections( filterName, data )` runs `applyFilters( filterName, [], data )` (`@wordpress/hooks`), normalizes descriptors (`id` string sanitized like account panel ids, first registration of a duplicate wins; `component` must be a `Function`; `order` `int`, default `10`, ties keep registration order), and `renderContributedSections( sections, { idPrefix, extraProps } )` wraps each in `#<idPrefix><id>` / `data-<derived-attr>="<id>"` (derived from `idPrefix`, e.g. `pixelgrade-styles-section-` → `data-styles-section`); `useSectionDeepLink( idPrefix )` scrolls/focuses/highlights the section named by `?section=<id>` on mount. Two consumers today:
  - `pixelgrade.adminHub.stylesSections` (Styles tab, `admin/src-modern/hub/tabs/Styles.js`) — components receive `{ stylesData, sectionId }`, container `#pixelgrade-styles-section-<id>` / `data-styles-section="<id>"`, `?tab=styles&section=<id>` deep-link. Reference consumer: Style Manager's Fonts section for the self-hosted cloud fonts control (`style-manager#183`), a companion outside the Plus/Assistant pair.
  - `pixelgrade.adminHub.setupSections` (Setup tab, `admin/src-modern/hub/tabs/Plugins.js`) — rendered at the BOTTOM of the tab, after the readiness summary and plugin list. Components receive `{ setupData, sectionId }` (`setupData` = the Setup tab's localized `window.pixelgradePlugins` payload), container `#pixelgrade-setup-section-<id>` / `data-setup-section="<id>"`, `?tab=plugins&section=<id>` deep-link. Shipped alongside `pixassist_setup_readiness_checks` below (pixelgrade-assistant#66).
- **`pixelgrade.adminHub.setupPluginRows` (JS filter)** — contributes component-only descriptors (`id`, `component`, `order`) inside the Setup tab's Recommended plugins list, using the same defensive normalization and `?section=<id>` deep-link behavior as contributed sections. Components receive `{ setupData, sectionId }`; wrappers are `#pixelgrade-setup-plugin-row-<id>` / `data-setup-plugin-row="<id>"`. The hub bootstrap advertises this independently deployable surface as `extensionSurfaces.setupPluginRows = 1`; companions must negotiate it and may fall back to the older `setupSections` surface when Assistant has not been updated yet. Assistant owns placement only. Companions own their view models, actions, nonces, capabilities, errors, and any commercial install path; package URLs, licenses, entitlement/WUpdates internals, and commercial installation logic must never enter Assistant. Reference consumer: Plus's `advanced-filtering` row.
- **`pixassist_setup_readiness_checks` (PHP filter) + `pixassist_normalize_contributed_setup_checks()`** — companions add rows to the Setup tab's readiness list (`includes/setup-readiness.php`): `apply_filters( 'pixassist_setup_readiness_checks', $checks, $facts )` runs inside `pixassist_get_setup_readiness_data()` right after `pixassist_build_setup_checks( $facts )` and BEFORE `pixassist_classify_setup_overall( $checks )`, so a contributed `blocked`/`warning` row affects the overall Ready/Needs attention/Blocked rollup. The filtered list is then defensively normalized (kept only if `id` is a non-empty `sanitize_key`'d string, deduped by id — first wins — and `label` is a non-empty string; `status` whitelisted to `ok`|`warning`|`blocked`, else `ok`; `group` defaults to `integrations`; `value`/`expected`/`why` coerced to strings; `action` kept only as `null` or `{ label, url }` (both non-empty strings); `items` coerced to an array), so a misbehaving companion cannot corrupt the tab. Pin test: `tests/setup-readiness-test.php`.
- **`pixelgrade/admin_hub/starters` (PHP filter) + `pixassist_get_admin_hub_starters()`** — Assistant seeds the Starter Sites tab from free `starterContent.demos`; companions append premium starter descriptors. Normalized item keys: `id`, `title`, `description`, `url`, `baseRestUrl`, `gate`, `image`, `previewUrl`, `badge`, `role`, `serves`, `source`, `order`, `capabilities`, `requiredPlugins`, `segments`, `applyPlan`. `gate` is item-level presentation (`''`|`plus`|`plus_licensed`), not access control. `role` is presentation too — `'starter'` (default; gets a Starter Sites card) vs `'library'` (a curated catalog: no card, and never importable as a whole site — `import_starter()` refuses a library with `not_a_starter` before it touches anything). **Which sections may list a source's material is `serves`, not `role`** — see the next bullet; the shared `window.pixelgradeStarterSites` payload carries every source, and each section filters it. Plus must also extend `pixassist_sce_allowed_demo_hosts` for any injected premium host outside Assistant's free demo config.
- **Design Library source capabilities (`includes/starter-sources.php`)** — a source descriptor carries two orthogonal facts. `role` is PRESENTATION (`starter` gets a card in the Starter Sites section; `library` does not). `serves` is CAPABILITY: which sections may list its material — any of `parts`, `content`, accepted as an array or a comma-separated string. `pixassist_get_starter_serves()` resolves it during normalization and `pixassist_starter_serves()` is what `pixassist_get_layout_units_sources()` (Site Parts) and `pixassist_get_content_patterns_sources()` (Page Patterns) each consult, so the two sections can never disagree about what a source offers. The derived default preserves every earlier behavior exactly: a starter serves both, and a library that declares nothing is a parts catalog. This is what lets a curated catalog of complete pages be a Page Patterns source without pretending to be a site anyone would apply whole, and keeps a parts catalog out of the page-level section. Pin tests: `tests/admin-starter-sites-test.php`, `tests/admin-content-patterns-test.php`, `tests/admin-layout-units-test.php`.
