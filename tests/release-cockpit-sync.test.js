...
test( 'release cockpit workflow parses the current version markers', () => {
	...
	assert.equal( outputs.plugin_version, '2.3.2' );
	assert.equal( outputs.singleton_version, '2.3.2' );
	assert.equal( outputs.readme_stable_tag, '2.3.2' );
	assert.equal( outputs.package_version, '2.3.2' );
